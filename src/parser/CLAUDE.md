...
## Purpose
Evaluates extracted video metadata against platform-specific feature criteria. Uses LangGraph ReAct agents backed by an LLM to query the SQLite database and produce structured pass/fail evaluations per feature.

## Key Files
* `parser_core.py`: `VideoInformationParser` — main entry point that resolves the platform evaluator and orchestrates the evaluation run.

...
## Key Files
* `extractor_core.py`: The `VideoInformationExtractor` class. It splits the video into shots, extracts frames, triggers DINO/SAM2/OCR, records raw frame detections and shot boundaries, calculates bounding box tracking metrics (velocity, growth, centrality), resolves object identities across different shots using DINOv2 embeddings, emits structured progress events through `ProgressReporter`, and collects GPT scene descriptions per shot into `scene_description_registry`.
...
* Artifact changes should keep run outputs under `artifacts/<run_id>/`; `max_artifact_files` caps per-frame PNGs only.

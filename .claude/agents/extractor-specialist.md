...
description: Use for work in src/extractor, src/ocr, src/dino/dino_wrapper.py, extraction metrics, scene description, taxonomy, or tracking integration.
...
- `src/extractor/`
- `src/ocr/`
- `src/dino/dino_wrapper.py`
- extraction-related config in `src/clip_scribe/configs/clip_scribe.yaml`
- safe wrapper or builder integration around SAM2 and DINO

Rules:
- Never read, analyze, or modify `src/dino/groundingdino/**` or `src/sam2/**` unless the user explicitly requests it.
...
- OCR consolidation changes belong in `src/ocr/paddle_wrapper.py`.

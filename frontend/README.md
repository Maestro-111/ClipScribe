# ClipScribe Frontend

A Vite + React + TypeScript single-page app for the ClipScribe web dashboard.
This now covers the jobs list, multi-video new-job form, batch/live job pages,
run inspector, ABCD export downloads, and run/job advisory chat described in
`docs/web-app-plan.md` §10.

## Stack (and why each piece exists)

| Tool | Role |
|---|---|
| **Vite** | Dev server (HMR) + production bundler. Also runs the `/api` dev proxy. |
| **React 19 + TypeScript (strict)** | UI, fully typed. |
| **TanStack Router** | File-based, type-safe routing. Files in `src/routes/` → URLs. |
| **TanStack Query** | Server-state cache: fetching, caching, refetch/polling, loading+error state. |
| **Zustand** | Available for future shared client state; the live-job page currently uses a local reducer. |
| **Tailwind v4** | Utility CSS via `@tailwindcss/vite`. No config file — theme lives in `src/styles.css`. |
| **openapi-typescript** | Generates `src/api/types.ts` from the API's OpenAPI schema. |
| **openapi-fetch** | Tiny typed HTTP client built on those types. |

> shadcn/ui (in the plan) is a component *generator*, not a runtime dep. Add it
> later with `pnpm dlx shadcn@latest init` — it vendors component source into the
> repo. Deferred to keep the bootstrap minimal.

## First run

```bash
# 1. Start the backend API (from backend/, another terminal).
#    Normal dashboard browsing needs the builder/DB dependencies loaded.
cd ../backend
uv run uvicorn app.main:app --reload

# 2. Install deps and generate the typed API client (needs the API running).
cd ../frontend
pnpm install
pnpm gen:api           # writes src/api/types.ts from http://localhost:8000/openapi.json

# 3. Start the dev server.
pnpm dev               # http://localhost:5173
```

`pnpm dev` generates `src/routeTree.gen.ts` on start (gitignored). If your editor
shows a red squiggle importing it in `main.tsx`, run `pnpm dev` once.

## How the pieces connect (the mental model)

```
Browser (:5173)
  └─ fetch("/api/jobs")            openapi-fetch, typed from src/api/types.ts
        └─ Vite proxy rewrites /api → :8000   (vite.config.ts; avoids CORS)
              └─ FastAPI /jobs      (backend/app/routes/jobs.py)
```

The live job page also opens `EventSource("/api/jobs/{id}/events")`, which
replays and tails Redis Stream progress events through the same Vite proxy.

- **Routing:** a file like `src/routes/runs.$runId.tsx` becomes `/runs/:runId`.
  The `$` marks a dynamic segment; `.` is a path separator. The Vite plugin
  regenerates `routeTree.gen.ts` whenever you add/rename a route file.
- **Type-safety across the stack:** `pnpm gen:api` turns the Python Pydantic
  models into a TS `paths` type. `openapi-fetch` is generic over it, so typed
  API calls reject wrong endpoints or malformed request bodies at compile time.
  Re-run `gen:api` after any backend API change (this is the anti-drift step,
  plan §9.12). Multipart uploads use `XMLHttpRequest` so the new-job form can
  show byte-level upload progress; a few lifecycle calls use plain `fetch`
  where it is simpler than wrapping them in `openapi-fetch`.
- **Data fetching:** ordinary JSON API reads/writes use the hooks in
  `src/api/hooks.ts` (`useJobs`, `useJob`, `useCreateJob`, …). Reads are
  `useQuery` (cached); writes are `useMutation` (invalidate the cache on
  success). Streaming chat uses `fetch` directly so it can read the POST response
  body as SSE frames, uploads use multipart `XMLHttpRequest` for progress
  events, lifecycle shortcuts use plain `fetch`, and export downloads are plain
  anchors.

## Layout

```
src/
  main.tsx              App entry: mounts React, QueryClient, RouterProvider.
  styles.css            Tailwind import.
  routeTree.gen.ts      GENERATED by the router plugin (gitignored).
  api/
    types.ts            GENERATED by openapi-typescript (`pnpm gen:api`).
    client.ts           openapi-fetch client + error unwrapping.
    hooks.ts            TanStack Query hooks over the client.
  components/
    ChatPanel.tsx       Run/job advisory chat, streamed over SSE.
    JobSidebar.tsx      New-job summary and expected output panels.
    Markdown.tsx        Dependency-free Markdown renderer for chat answers.
    PipelineAnimation.tsx  Decorative pipeline companion for the new-job page.
    ui.tsx              Shared UI, including pagination and the ABCD export menu.
  lib/format.ts         Display helpers.
  routes/
    __root.tsx          App shell (nav + <Outlet/>).
    index.tsx           "/"            Jobs list.
    jobs.$jobId.tsx     "/jobs/:jobId" Live job progress.
    jobs.new.tsx        "/jobs/new"    New-job form.
    runs.$runId.tsx     "/runs/:runId" Run inspector.
```

## Current UI scope

- Jobs list: status filter, polling, server-side offset pagination, parent batch rows with child summaries, terminal batch duration from merged child run intervals, running-run progress bars, links to completed jobs' batch/run view, cancel/retry actions, job deletion, and a Scribe empty state.
- New job: create user-facing `full` jobs from picked registry videos and/or locally queued files. Files are not uploaded until "Create job"; folder selection queues every allowed video in the folder, then submit uploads them with a byte-progress stepper, merges returned storage keys, and creates the batch. `extract` and parser-only `parse` stay developer paths outside the form. Brand and call-to-action fields are optional, with warnings when omitted because the matching criteria have no terms to check. A draggable Scribe companion reacts to queued videos and submit state on wide screens.
- Job pages: parent jobs show the batch's child runs with client-side pagination, per-child durations, job-level advisory chat, merged active duration once stopped, and an "Export all" CSV/XLSX menu once at least one child completes; child/leaf jobs show the SSE-driven progress bar, phase tree, current-shot panel, log tail from `GET /jobs/{job_id}/events`, and draggable Scribe narration.
- Run inspector: raw video playback with SVG boxes for tracked objects and OCR text, batch sibling navigation, back-to-batch navigation, shot/audio timeline, parser criteria table with CSV/XLSX export, tracked video download, and run-scoped advisory chat.
- Not yet built: the fuller inspector layer controls from the plan and failed-criterion "ask about this" shortcuts.

## Scripts

- `pnpm dev` — dev server with HMR.
- `pnpm build` — typecheck + production build to `dist/`.
- `pnpm typecheck` — types only (no build).
- `pnpm gen:api` — regenerate `src/api/types.ts` from the running API.
